...
const userHTML = `<!doctype html><html><head><meta charset="utf-8"><title>User Panel</title><style>body{font-family:Arial;margin:20px;max-width:1100px;background:#fafafa}.card{border:1px solid #ddd;padding:14px;border-radius:8px;margin-bottom:14px;background:#fff}button{padding:7px 10px;margin-right:6px}input{padding:6px}input[type=text],input[type=password]{width:280px}.skills{display:grid;grid-template-columns:repeat(2,minmax(320px,1fr));gap:12px}.skill{border:1px solid #e5e5e5;border-radius:8px;padding:10px;background:#fcfcfc}code{background:#f0f0f0;padding:2px 4px;border-radius:4px}</style></head><body><h2>User Panel</h2><p><a href="/admin">Go to Admin Panel</a></p><div class="card"><h3>Login</h3><input id="u" placeholder="username"/><input id="p" type="password" placeholder="password"/><button onclick="login()">Login</button><button onclick="logout()">Logout</button><span id="auth"></span></div><div class="card"><h3>My Skills (select/deselect tools)</h3><div class="skills" id="skills"></div></div><script>
function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
async function j(u,o={}){const r=await fetch(u,Object.assign({headers:{'content-type':'application/json'}},o));const t=await r.text();if(!r.ok) throw new Error(t); return t?JSON.parse(t):{}}
window.__s=null;
